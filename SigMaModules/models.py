...
class SigMa():
    ...
    def __init__(self, args) -> None:
        ...
        directories = ['reference', 'query', 'search',
                       'plots', 'regions', 'checkv', 'phispy']
    ...
    def handle_queries(self):
        ...
        for query_type, query_dataset_path in zip(self.args.query_type, self.args.query):
            ...
            qi += 1
            done_q = f"{qi}/{q_num}"
            log_progress(f"[{done_q}] Query: {query_dataset_path}",
                         msglevel=0, loglevel="INFO")
            self.queries.append(
                # create Query object
                Query(
            ...
            for record_query in self.queries[-1].get_record_queries():
                ...
                if not record_query.has_nt() and not record_query.has_aa():
                    log_progress(
                        f"[{done_q} - {done_rq}] {record_query} skipped - no sequences", msglevel=1, loglevel="DEBUG")
                    continue
                elif record_query.len() < self.args.min_nt_sig:
                    log_progress(
                        f"[{done_q} - {done_rq}] {record_query} skipped - too short)", msglevel=1, loglevel="DEBUG")
                    continue
                else:
                    log_progress(
                        f"[{done_q} - {done_rq}] {record_query}", msglevel=1, loglevel="DEBUG")
                ...
                if record_query.has_nt():
                    ...
                    log_progress(
                        f"[{done_q} - {done_rq}] {record_query} has no nucleotide sequences", msglevel=2, loglevel="WARNING")
                if record_query.has_aa():
                    query_aas.extend(record_query.get_fasta_aa())
                else:
                    log_progress(
                        f"[{done_q} - {done_rq}] {record_query} has no proteins", msglevel=2, loglevel="WARNING")
            ...
            if len(query_nts) > 0:
                ...
                log_progress(
                    f"0 nucleotide sequences in {self.queries[-1]}. Not writing.", msglevel=2, loglevel="WARNING")
            if len(query_aas) > 0:
                self.write_fastas(query_aas, query_aa_path)
            else:
                log_progress(
                    f"0 proteins in {self.queries[-1]}. Not writing.", msglevel=2, loglevel="WARNING")
            ...
            for target in sorted(self.targets, key=lambda x: {'fasta_nt': 0, 'fasta_aa': 1, 'hmm': 2, 'mmseqs_db': 3}[x.type]):
                ...
                if target.type == 'fasta_nt':
                    if len(query_nts) > 0:
                        output_path = target.search(
                            query_nt_path, output_prefix)
                else:
                    if len(query_aas) > 0:
                        output_path = target.search(
                            query_aa_path, output_prefix)
                ...
                nt_signal_array, aa_signal_array = target.read_output(
                    self.queries[-1], output_path)  # returns a dict of arrays per record_id
                for record_query in self.queries[-1].get_record_queries():
                    rqid = record_query.get_id()
                    signal_group = 'nt_based'
                    if rqid in nt_signal_array and nt_signal_array[rqid] is not None:
                        record_query.add_signal(
                            signal_group, target.name, nt_signal_array[rqid])
                    if rqid in aa_signal_array and aa_signal_array[rqid] is not None and target.type != 'fasta_nt':
                        signal_group = 'aa_based'
                        record_query.add_signal(
                            signal_group, target.name, aa_signal_array[rqid])
            ...
            if self.args.ext_tools and 'phispy' in self.args.ext_tools:
                ...
                for record_query in self.queries[-1].get_record_queries():
                    ...
                    if rqid in nt_signal_array and nt_signal_array[rqid] is not None:
                        record_query.add_signal(
                            'nt_based', 'phispy', nt_signal_array[rqid])
            ...
            for record_query in self.queries[-1].get_record_queries():
                ...
                # EVALUATE SIGNALS
                log_progress(
                    f"[{done_q} - {done_rq}] Evaluating {record_query}", msglevel=1, loglevel="DEBUG")
                record_query.evaluate(self.args.max_nt_gap, self.args.min_nt_sig, self.args.max_aa_gap,
                                      self.args.min_aa_sig, self.args.min_sig_frac, self.args.sig_groups)
            ...
            log_progress(
                f"{rm_num} merged non-overlapping regions were identified based on {r_num - rm_num} regions", msglevel=1, loglevel="INFO")
    ...
    def run_checkv(self) -> None:
        ...
        log_progress("Processing CheckV output data",
                     msglevel=0, loglevel="INFO")
        ...
        checkv = df.merge(cdf, on=ckeys, how='left').to_dict(
    ...
    def run_phispy(self, query, prefix) -> Dict[str, Dict[str, np.ndarray]]:
        ...
        log_progress("Processing PhiSpy output data",
                     msglevel=1, loglevel="DEBUG")
        phispy_output = os.path.join(
            self.dirs['phispy'], f"{prefix}_prophage_coordinates.tsv")
        if not os.path.exists(phispy_output):
            log_progress(
                f"PhiSpy output file {phispy_output} not found!", loglevel="WARNING")
            return {}
        else:
            df = pd.read_csv(phispy_output, sep='\t', names=['pp', 'contig', 'start', 'end', 'start_att1',
                             'start_att2', 'end_att1', 'end_att2', 'start_att', 'end_att', 'comment'], keep_default_na=False)

            nt_signal_array = {}
            for record_id in df['contig'].unique():
                nt_signal_array[record_id] = np.zeros(
                    query.get_record_len(record_id))
                for i, row in df.loc[df['contig'] == record_id].iterrows():
                    nt_signal_array[record_id][int(
                        row['start']) - 1: int(row['end'])] += 1
    ...
    def list_regions(self, regions: List) -> None:
        ...
        for region in regions:
            log_progress(f"{region} - {region.status}",
    ...
    def write_regions(self, regions: List, group: str, format: Union[str, List[str]] = 'fasta') -> None:
        ...
        for f in format:
            if f == 'fasta':
                ...
                self.write_fastas([region.to_fasta_nt()
                                  for region in regions], regions_file_path)
            elif f == 'genbank':
                regions_file_path = os.path.join(regions_dir, f"{group}.gb")
                self.write_genbanks([region.to_genbank()
                                    for region in regions], regions_file_path)
    ...
    def write_fastas(self, fastas: Union[str, List[str]], out_path: str) -> None:
        ...
        with open(out_path, 'w') as out:
            for fasta in fastas:
                ...
                cnt += fasta.count('>')
        log_progress(
            f"wrote {cnt} sequence{'s' if cnt > 1 else ''} to {out_path}", msglevel=1, loglevel="DEBUG")
    ...
    def write_genbanks(self, seqrecs: Union[SeqRecord, List[SeqRecord]], out_path: str) -> None:
        ...
        log_progress(
            f"wrote {len(seqrecs)} sequence{'s' if len(seqrecs) > 1 else ''} to {out_path}", msglevel=1, loglevel="DEBUG")
    ...
    def write_summary(self, sep: str = '\t') -> None:
        ...
        # prepare output file paths
        gen_sum_path = os.path.join(
            self.args.outdir, 'summary.all_regions.tsv')
        cand_sum_path = os.path.join(
            self.args.outdir, 'summary.candidates.tsv')
        ...
        log_progress(
            f"wrote summary to {gen_sum_path}", msglevel=1, loglevel="INFO")

        with open(cand_sum_path, 'w') as out:
            for region in self.filter_regions(sig_groups=['merged'], sig_sources=['merged']):
                out.write(sep.join(map(str, region.to_list())) + "\n")
        log_progress(
            f"wrote summary to {cand_sum_path}", msglevel=1, loglevel="INFO")

        with open(ver_sum_path, 'w') as out:
            for region in self.hq_regions:
                out.write(sep.join(map(str, region.to_list())) + "\n")
        log_progress(
            f"wrote summary to {ver_sum_path}", msglevel=1, loglevel="INFO")
...
class Target(Input):
    ...
    def search(self, query_path: str, output_prefix: str) -> str:
        ...
        self.output_paths.append(
            f"{output_prefix}.{self.name}.{self.type}.tsv")
        output_path = self.output_paths[-1]

        if self.params.reuse and os.path.exists(output_path):
            log_progress(f"reusing {output_path}",
                         msglevel=1, loglevel="DEBUG")
            return output_path
        else:
            log_progress(
                f"searching against {self.name}", msglevel=1, loglevel="DEBUG")

        if self.type == 'fasta_nt':
            cmd = 'blastn -query {} -db {} -out {} -outfmt "6 qaccver saccver pident length qstart qend evalue" -evalue {} -perc_identity {} -num_threads {} -max_target_seqs 10000'.format(
                query_path, self.db_path, output_path, self.params.nt_evalue, self.params.nt_pident, self.params.threads)
            call_process(cmd)

        elif self.type == 'fasta_aa' or self.type == 'mmseqs_db':
            ...
            # make a search
            if self.type == 'mmseqs_db':
                cmd = f"mmseqs search --cov-mode 0 --max-seqs 3 -c {self.params.prof_cov} -s {self.params.prof_sens} --threads {self.params.threads} -e {self.params.prof_evalue} {protein_db_path} {self.db_path} {results_path} {tmp_path} -v 2"
            elif self.type == 'fasta_aa':
                cmd = f"mmseqs search --cov-mode 0 --max-seqs 3 -c {self.params.aa_cov} -s {self.params.prof_sens} --threads {self.params.threads} -e {self.params.aa_evalue} --min-seq-id {self.params.aa_pident} {protein_db_path} {self.db_path} {results_path} {tmp_path} -v 2"
            call_process(cmd)

            # create a results tsv file
            cmd = f"mmseqs createtsv {protein_db_path} {self.db_path} {results_path} {output_path} -v 2"
            call_process(cmd)
    ...
    def read_output(self, query, output_path: str) -> Tuple[Dict[str, Dict[str, np.ndarray]], Dict[str, Dict[str, np.ndarray]]]:
        """
        Read BLAST or MMseqs2 output and return information about regions with signal
        :param file_path: path to search output file
        :param query: Query object
            ...
            for line in open(output_path, 'r'):
                if line.startswith('#'):
                    ...
                    if int(length) >= self.params.nt_length:
                        nt_signal_array[record_id][int(
                            qstart) - 1: int(qend)] += signal

        elif self.type == 'mmseqs_db' or self.type == 'fasta_aa':
            for line in open(output_path, 'r'):
                if line.startswith('#'):
                    continue
                else:
                    sseqid, qseqid, alnScore, seqIdentity, eVal, qStart, qEnd, qLen, tStart, tEnd, tLen = line.strip().split('\t')
                    ...
                    if record_id not in nt_signal_array:
                        nt_signal_array[record_id] = np.zeros(
                            query.get_record_len(record_id))
                        aa_signal_array[record_id] = np.zeros(
                            query.get_record_cds_num(record_id))

                    # record nt signal
                    nt_signal_array[record_id][int(
                        start) - 1: int(end)] += signal
                    # record aa signal
                    aa_signal_array[record_id][query.get_record_cds_order_num(
                        record_id, protein_id)] += signal
...
class Query(Input):
    ...
    def __init__(self, file_path: str, type: str, name: str = None):
        ...
        if self.type == 'fasta':
            self.records = [RecordQuery(record)
                            for record in parse_fasta(self.file_path)]
            ...
            for record in sorted(parse_genbank(self.file_path), key=lambda x: len(x.seq), reverse=True):
                ...
                log_progress(
                    f"{record.id}: {self.records[-1].len()} bps and {len(self.cdss) - cnt} CDSs", msglevel=1, loglevel="DEBUG")
                cnt = len(self.cdss)
...
class Record():
    ...
    def __init__(self, record: SeqRecord, file_path: str):
        ...
        self.cdss_idx: Dict[str, int] = {
            cds.qualifiers['protein_id'][0]: i for i, cds in enumerate(self.cdss)}
        self.nt_path: str = ""
        self.aa_path: str = ""
    ...
    def get_cds_order_num(self, cds_id: str) -> int:
        ...
        try:
            ...
            log_progress(
                f"Could not find CDS {cds_id} in record {self.record.id}", msglevel=1, loglevel="ERROR")
            log_progress(
                f"Available CDSs: {self.cdss_idx.items()}", msglevel=1, loglevel="ERROR")
            log_progress(
                f"Available CDSs: {[cds.qualifiers['protein_id'][0] for cds in self.cdss]}", msglevel=1, loglevel="ERROR")
            raise KeyError
            ...
            if feature.type == ftype:
                if ftype == 'CDS':
                    if 'translation' not in feature.qualifiers:
                        feature.qualifiers['translation'] = [feature.extract(
                            self.record.seq).translate(table=11, to_stop=True)]
                    if 'protein_id' not in feature.qualifiers:
                        feature.qualifiers['protein_id'] = [
                            f'{self.record.id}_ft_{fcnt:06d}']
                    ...
                    if len(feature.qualifiers['translation'][0]) < min_cds_len:
                        log_progress(
                            f"CDS {feature.qualifiers['protein_id'][-1]} is shorter than {min_cds_len} aa which breaks MMseqs2 run. Skipping.", msglevel=1, loglevel="DEBUG")
                        continue
...
class RecordQuery(Record):
    ...
    def print_signal_summary(self) -> None:
        ...
        for signal_group, signal_names in self.signals.items():
            if len(self.signals[signal_group]) == 0:
                continue
            log_progress(
                f"{signal_group} signal group for {self.name}:", msglevel=1, loglevel='DEBUG')
            for signal_name, signal in signal_names.items():
                log_progress(
                    f"{signal_name}: {sum([1 if x else 0 for x in signal])}/{signal.size} of total {sum(signal)}", msglevel=2, loglevel='DEBUG')
    ...
    def plot(self, output_dir: str, plot_type: str) -> None:
        ...
        if plot_type == 'artemis':
            output_path = os.path.join(
                output_dir, f"{self.record.id}.artemis.plot.gz")
            write_df_to_artemis(self._get_signal_df(), output_path)

        if plot_type == 'plotly':
            output_path = os.path.join(
                output_dir, f"{self.record.id}.plotly.json")
    ...
    def combine_signals(self) -> None:
        ...
        for signal_group in self.signals.keys():
            combined_array = np.zeros(
                self.len() if signal_group == 'nt_based' else self.get_cdss_num())
    ...
    def evaluate(self, max_nt_gap: int, min_nt_signals: int, max_aa_gap: int, min_aa_signals: int, min_sig_frac: float, sig_groups: List[str]) -> None:
        ...
        """
        log_progress(f"Using " + ", ".join(sig_groups) +
                     " signal groups.", msglevel=1, loglevel='DEBUG')
        ...
        for signal_group in self.signals.keys():
            ...
            log_progress(f"{signal_group} evaluation of {self.name}",
                         msglevel=1, loglevel='DEBUG')
            for target, signal_array in self.signals[signal_group].items():
                log_progress(
                    f"{np.count_nonzero(signal_array)} {'positions' if signal_group == 'nt_based' else 'proteins'} based on {target}", msglevel=2, loglevel='DEBUG')
                        ...
                        if (gap_size > max_gap_size):
                            ...
                            if (pos_len >= min_sig_signals) and (pos_len / (len(region_values) - gap_size + 1) >= min_signal_frac):
                                if signal_group == 'nt_based':
                                    ...
                                    region_end = self.cdss[i_gap -
                                                           max_gap_size + 1].location.nofuzzy_end
                                candidate_region = Region(
                                    ...
                                    signal=signal_array[i_pos: i_gap -
                                                        max_gap_size + 1],
                                    signal_group=signal_group,
                                    rno=len(self.regions[signal_group]) + 1
                                )

                                self.regions[signal_group].append(
                                    candidate_region)
                                log_progress(
                                    f"{len(self.regions[signal_group])}: {candidate_region}", msglevel=3, loglevel='DEBUG')
                            else:
                                # thresholds unmet
                                pass
    ...
    def merge_regions(self) -> None:
        ...
        log_progress(
            f"merging overlapping regions of {self.name}", msglevel=1, loglevel='DEBUG')

        # get all regions
        regions = sorted(
            self.regions['nt_based'] + self.regions['aa_based'], key=lambda x: [x.start, -x.len()])
        ...
        for region_start, region_end in ranges:
            ...
            self.regions[signal_group].append(candidate_region)
            log_progress(
                f"{len(self.regions[signal_group])}: {candidate_region}", msglevel=2, loglevel='DEBUG')

        log_progress(
            f"found {len(self.regions['merged'])} non-overlapping regions: {len(self.regions['nt_based'])} based on nt signal group and {len(self.regions['aa_based'])} based on aa signal group", msglevel=1, loglevel='DEBUG')
    ...
    def update_checkv(self, d: dict):
        ...
        if self.status.startswith('CheckV'):
            ...
            if 'host' in region_types:  # extract viral region
                ...
                start, end = map(
                    int, region_coords_bp[region_types.index('viral')].split('-'))
                start -= 1
                # coords
                self.start += start
