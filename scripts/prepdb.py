...
def meshclust_cluster(fasta_path: str, clusters_path: str, nt_id: float, threads: int) -> List[str]:
    ...
    log_progress(
        f"MeShClust: {C_cnt} clusters created, {M_cnt} cluster members, {E_cnt} cluster extended members", loglevel="INFO", msglevel=1)
...
def main():
    ...
    parser.add_argument('--gb', help='GenBank file',
                        nargs='+', metavar='<path>')
    parser.add_argument('--fasta_aa', help='Protein FASTA file',
                        nargs='+', metavar='<path>')
    parser.add_argument(
        '--fasta_nt', help='Nucleotide FASTA file', nargs='+', metavar='<path>')
    parser.add_argument('--dbdir', help='Output directory',
                        metavar='<path>', required=True)
    parser.add_argument(
        '--sens', help='Sensitivity (0.0-7.5) [%(default)0.1f]', default=7.5, metavar=' ', type=float)
    parser.add_argument(
        '--id', help='Identity threshold (0.0-1.0) [%(default)0.2f]', default=0.9, metavar=' ', type=float)
    parser.add_argument(
        '--nt_id', help='Nucleotide identity threshold (0.0-1.0) [%(default)0.2f]', default=0.8, metavar=' ', type=float)
    parser.add_argument(
        '--cov', help='Minimum coverage (0.0-1.0) [%(default)0.2f]', default=0.8, metavar=' ', type=float)
    parser.add_argument(
        '--cov_mode', help='Coverage mode [%(default)i]', default=0, metavar=' ', type=int)
    parser.add_argument(
        '--evalue', help='Maximum e-value [%(default)1.0e]', default=1e-3, metavar=' ', type=float)
    parser.add_argument(
        '--threads', help='Number of threads [%(default)i]', default=4, metavar=' ', type=int)
    parser.add_argument('--tmp', help='Temporary directory',
                        metavar='<path>', required=True)
    parser.add_argument('--log', help='Log file', metavar='<path>')
    parser.add_argument('--logging', help='Logging level', metavar='<level>',
                        default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'])
    parser.add_argument(
        '--simple_log', help='Just print messages without time and levels', action='store_true')
    ...
    if args.log:
        args.logger = create_logger(
            log_path=args.log, log_level=args.logging, simple=args.simple_log)
    else:
        args.logger = create_logger(log_path=os.path.join(
            args.dbdir, "prepdb.log"), log_level=args.logging, simple=args.simple_log)

    log_progress("Preparing reference datasets for SigMa.",
                 loglevel="INFO", msglevel=0)
    ...
    if args.gb:
        for gb in args.gb:
            ...
            log_progress(
                f"Using name '{dbname}' for the database", loglevel="INFO", msglevel=1)
            ...
            for record in records:
                try:
                    if len(record.seq) == 0:
                        log_progress(
                            f"Missing sequence for record {record.id}. Skipping.", loglevel="WARNING", msglevel=1)
                        continue
                except:
                    log_progress(
                        f"Problem with nucleotide sequence for record {record.id}. Skipping.", loglevel="WARNING", msglevel=1)
                    continue
                # write nt
                nt_cnt += 1
                ...
                try:
                    ...
                    log_progress(
                        f"Problem with nucleotide sequence for record {record.id}. Skipping. Error: {e}", loglevel="WARNING", msglevel=1)
                    continue
                # write aa
                r_aa_cnt = 0
                for f in record.features:
                    if f.type == 'CDS':
                        ...
                        aa_seq = f.qualifiers['translation'][0] if 'translation' in f.qualifiers else f.extract(
                            record.seq).translate(table=11, to_stop=True)
                        try:
                            aa_out.write(f"{aa_head}\n{aa_seq}\n")
                        except Exception as e:
                            log_progress(
                                f"Problem with protein sequence for record {record.id}: {f}. Skipping. Error: {e}", loglevel="WARNING", msglevel=1)
                            continue
                log_progress(
                    f"Record {record.id} had {r_aa_cnt} CDS features", loglevel="INFO", msglevel=1)
            # close files
            aa_out.close()
            nt_out.close()
            ...
            if aa_cnt > 0:
                log_progress(
                    f"{aa_cnt} protein sequences written to {aa_file}", loglevel="INFO", msglevel=1)
                aa_fastas.append(aa_file)
            else:
                log_progress(
                    f"There were no CDS features in provided GenBank file.", loglevel="WARNING", msglevel=1)
            if nt_cnt > 0:
                log_progress(
                    f"{nt_cnt} nucleotide sequences written to {nt_file}", loglevel="INFO", msglevel=1)
    ...
    if aa_fastas:
        ...
        for aa_fasta in aa_fastas:
            ...
            rep_seq_fasta = mmseqs_cluster(
                dbname, aa_fasta, args.tmp, args.sens, args.cov, args.cov_mode, args.id, args.evalue, args.threads)
            records = parse_fasta(rep_seq_fasta)
            log_progress(
                f"MMSeqs: {len(records)} clusters created", loglevel="INFO", msglevel=1)
    ...
    if nt_fastas:
        ...
        for nt_fasta in nt_fastas:
            ...
            clusters_file = os.path.join(
                args.tmp, f"{dbname}.meshclusters.tsv")
            clusters_ids = meshclust_cluster(
                nt_fasta, clusters_file, args.nt_id, args.threads)
            write_fasta([record for record in parse_fasta(nt_fasta)
                        if record.id in clusters_ids], nt_cl_fasta)

    log_progress(f"Removing temporary directory {args.tmp}", loglevel="INFO")
    shutil.rmtree(args.tmp)

    log_progress("Done! You can now run use output files for SigMa!",
                 loglevel="INFO", msglevel=0)
